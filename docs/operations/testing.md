...
场景导入测试还应覆盖：缺失业务 ID 自动生成且跨对象不重复、已有 ID 保持不变；车辆燃料/油耗/每日成本缺失时界面留空且请求省略字段；有效仓库、Agent 或 Ticket 缺少地址或坐标时 Gateway 创建校验拒绝提交。
...
* Gateway 对话草稿中的 `09:00` 等纯时间会结合规划日期规范化为 `yyyy-MM-dd HH:mm:ss`；无日期、非法日期时间、含 `T`/时区、无秒值和跨日歧义均有覆盖，纯时间不会进入最终创建请求；
* 草稿已有地址但缺少坐标时自动正向解析、已有坐标但缺少可读地址时自动逆向解析，地址和坐标完整时不重复请求；解析失败保留原始输入并阻止进入创建确认；
* 组件场景大纲仅包含业务汇总和完整性状态，不暴露内部 ID、经纬度、原始请求路径或完整约束参数；
