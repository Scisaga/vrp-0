...
The three Gateway-imported metadata files belong in that engine commit's `gateway/` directory and are imported from the configured Git tag. Gateway's metadata path configuration must point to this directory. The reference `request-schema.json`, documentation and test fixtures belong in `docs/integrations/gateway/`; Gateway reads the request contract from `docs/openapi.yaml`, not the reference request schema.
