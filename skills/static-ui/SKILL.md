---
name: static-ui
description: Use this skill when maintaining or extending the repository's static UI under static/, especially for TailwindCSS v4 + Alpine.js app shell pages, local-only assets, page fragments under pages/, and Node-only build-time CSS compilation without introducing bundlers or external CDNs.
---
...
- 高优先级结构约束（必须遵守，优先级高于“做得好看”）：
  1) 禁止同语义容器反复套壳
     ...
     - 当一个面板内包含多个内容分区时，优先使用 `divide-y` / `border-t` / `grid` 划分区域，而不是继续套 `rounded-* + border` 子卡片
     - `section` 用于语义分区，默认应贴着父面板表面展开；只有“统计 tile / 缩略图 / 弹窗主体 / 强提醒块”这类局部内容，才允许使用独立卡片样式
     - 对“源码 / SQL / JSON / 日志 / Payload / 长 `pre`”这类技术阅读区，默认采用“标题栏 + 正文”单面板；正文直接贴着面板表面展开，不要在正文外再套一层带 `rounded / border / shadow / mt-* / p-*` 的内层代码卡片
     - 反例：详情页里 `panel -> section -> card -> list` 连续 4 层包裹，只是为了制造边框和留白
  2) 二级列表和长内容必须收敛在当前视区内
